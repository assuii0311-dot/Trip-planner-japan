...
| | 스페인 | 일본 (도쿄) |
|---|---|---|
| 여행의 축 | 도시 사이를 옮겨 다닌다 | 한 곳에 묵고 지역을 돈다 |
| 단위 | 도시 60곳 | 도쿄 안의 지역 11곳 + 근교 7곳 |
| 이동 시간 | Renfe 시간표 + 실측 51구간 | 지하철 역~역 실측 51구간 + 20분 |

같은 엔진이다. 일본을 붙이며 바뀐 것은 [docs/24-japan-data-plan.md](docs/24-japan-data-plan.md) 에 있다 —
온천 테마, 지역 등급(`tier: 'district'`), 나라별 문앞~문앞 오버헤드, 구간별 화면.
...
앱 코드는 수정하지 않습니다. 일본은 `node pipeline/collect.mjs japan` 이고, 손으로 넣는 항목은
`registry/japan-extras.mjs`, 한국어 설명은 `pipeline/ko/japan/` 입니다.

```bash
cd app
npx tsx scripts/district-check.mjs     # 도쿄 모델 — 지역 등급·오버헤드·온천 숙박
```
